...
class PostgresqlConnector(DatabaseConnector):
    def connect(self) -> None:
        logger.info(
            f"[{self.__class__.__name__}] Connect to '{self.config.engine}' database: {self.config.connection_info.database}, table: {self.config.connection_info.tb_or_collection}."
        )

        del self.config.connection_info.tb_or_collection
