...
class GoogleDriveDownloader(Downloader):
    def validate(self) -> None:
        ...
        if self.config.options.item_type not in ("file", "folder"):
            raise ValueError(
                f"[{self.__class__.__name__}] type must be 'file' or 'folder'"
            )

        if self.config.options.item_type == "folder":
            if not self.config.url and not self.config.id:
                raise ValueError(
                    f"[{self.__class__.__name__}] Need provide url or id for folder download."
                )
        else:
            if not self.config.url:
                raise ValueError(
                    f"[{self.__class__.__name__}] Need provide url for file download."
                )
